...
from .Helsinki import helsinki_generator
...
def text_to_image(prompt: str, output_path: str, num_inference_steps: int, width: int, height: int, n_prompt="") -> None:
    ...
    options = TextToImageOptions(prompt=prompt, output_path=output_path, num_inference_steps=num_inference_steps,
                                 width=width, height=height, n_prompt=n_prompt)
...
def depth_to_image(prompt: str, img_url: str, output_path: str, num_inference_steps: int, width: int,
                   height: int, n_prompt="") -> None:
    stable_diffusion_instance = StableDiffusionAI()
    options = DepthToImageOptions(prompt=prompt, img_url=img_url, output_path=output_path,
                                  num_inference_steps=num_inference_steps,
                                  width=width, height=height, n_prompt=n_prompt)
